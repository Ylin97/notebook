...
1. 最安全的分配和使用动态内存的方法是使用`make_shared`标准库函数，而不是使用`new`。
...
4. 释放一个空指针`nullptr`或`NULL`是合法的。

5. `shared_ptr`接受内置指针的构造函数是**`explicit`**的，因此必须使用直接初始化形式。默认情况下，用于初始化`shared_ptr`的内置指针必须指向动态内存，因为`shared_ptr`默认使用`delete`来释放内存。如果要使用非指向动态内存的指针来初始化`shared_ptr`，那么必须提供额外的删除器来代替默认的`delete`。

   | 构造函数                                       | 描述                                                         |
   | ---------------------------------------------- | ------------------------------------------------------------ |
   | `shared_ptr<T> p(q)`                           | `p`管理内置指针`q`所指向的对象；`q`必须指向new分配的内存，且能够转换为`T*`类型 |
   | `shared_ptr<T> p(u)`                           | `p`从`unique_ptr u`那里接管了对象的所有权；将`u`置为空       |
   | `shared_ptr<T> p(q, d)`                        | `p`接管内置指针`q`所指向的对象的所有权。`q`必须能转换为`T*`类型。`p`将使用可调用对象`d`来代替`delete` |
   | `shared_ptr<T> p(p2, d)`                       | `p`是`shared_ptr p2`的拷贝，唯一的区别是`p`将用可调用对象`d`来代替`delete` |
   | `p.reset()`<br>`p.reset(q)`<br>`p.reset(q, d)` | 若`p`是唯一指向其对象的`shared_ptr`，则`reset`会释放此对象。若传递了可选的参数内置指针`q`，则会令`p`指向`q`，否则会将`p`置为空。若还传递了参数`d`，将会调用`d`而不是`delete`来释放`q` |
   ...
   shared_ptr<int> clone(int p) {
       return new int(p); // 错误：试图隐式转换为shared_ptr<int>
   }
...
6. `get`用来将指针的访问权限传递给代码，你只有在确定代码不会`delete`指针的情况下，才能使用`get`。
7. 不要混用普通指针和智能指针，也不要使用`get`获取的内置指针初始化另一个智能指针或为智能指针赋值。
